...
def _open_db(db_path: str | Path) -> Database:
    """按路径开一个只读使用的 Database。

    Database.__init__ 不暴露 check_same_thread，这里直接建连接后塞回去：所有工具
    跑在事件循环单线程，本来就同线程；check_same_thread=False 仅作冗余保险（纯只读）。
    """
    db = Database.__new__(Database)
    db.path = db_path
    db_arg = db_path if db_path == ":memory:" else str(db_path)
    db._conn = sqlite3.connect(db_arg, check_same_thread=False)
